"""
sources/nouns/generate.py
─────────────────────────
Reads source/nouns.csv and config.yaml, transforms the data into the
project-wide standardised schema, and writes the result to
../../spreadsheets/nouns.csv.
...
SOURCE_CSV = os.path.join(SCRIPT_DIR, "source", "nouns.csv")
...
def transform_row(raw: dict, index: int, config: dict) -> dict:
    """
    Map a row from source/nouns.csv to the standardised spreadsheet schema.
    See spreadsheets/schema.md for full column definitions.
    """
    card_id = build_id(index)
    tags = " ".join(config.get("tags", []))

    # Append gender tag so cards can be filtered by gender in Anki
    gender = raw.get("gender", "").strip()
    if gender:
        tags = f"{tags} gender_{gender}"
